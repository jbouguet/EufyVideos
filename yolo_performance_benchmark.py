...
class YoloBenchmark:
    ...
    def benchmark_detector(
        self, detector_class, model_name: str, implementation: str, num_frames: int = 50
    ) -> Dict[str, Any]:
        """
        ...
        detections = detector.detect_objects(
            video_path=self.test_video_path, num_frames=num_frames
        )
    ...
    def run_full_benchmark(
        ...
        logger.info(
            f"GPU acceleration: {'MPS' if torch.backends.mps.is_available() else 'CUDA' if torch.cuda.is_available() else 'None'}"
        )
        logger.info("")
        ...
        for model_file, model_desc in models:
            ...
            try:
                ...
                # Calculate speedup
                det_speedup = (
                    optimized_result["detection_fps"] / original_result["detection_fps"]
                    if original_result["detection_fps"] > 0
                    else 0
                )
                track_speedup = (
                    optimized_result["tracking_fps"] / original_result["tracking_fps"]
                    if original_result["tracking_fps"] > 0
                    else 0
                )

                logger.info(
                    f"  Speedup - Detection: {det_speedup:.2f}x, Tracking: {track_speedup:.2f}x"
                )
    ...
    def save_results(
        self, results: Dict[str, Any], output_file: str = "yolo_benchmark_results.json"
    ):
    ...
    def print_summary(self, results: Dict[str, Any]):
        ...
        print(
            f"{'Model':<15} {'Task':<10} {'Original':<12} {'Optimized':<12} {'Speedup':<10}"
        )
        print("-" * 70)
        ...
        for i, original in enumerate(results["original"]):
            if i < len(results["optimized"]):
                ...
                det_speedup = (
                    optimized["detection_fps"] / original["detection_fps"]
                    if original["detection_fps"] > 0
                    else 0
                )
                print(
                    f"{model:<15} {'Detection':<10} {original['detection_fps']:<12.1f} "
                    f"{optimized['detection_fps']:<12.1f} {det_speedup:<10.2f}x"
                )

                # Tracking comparison
                track_speedup = (
                    optimized["tracking_fps"] / original["tracking_fps"]
                    if original["tracking_fps"] > 0
                    else 0
                )
                print(
                    f"{'':<15} {'Tracking':<10} {original['tracking_fps']:<12.1f} "
...
def main():
    ...
    # Test video path
    test_video = (
        "/Volumes/SSK Drive/record/Batch010/T8600P1024260D5E_20241118084615.mp4"
    )
